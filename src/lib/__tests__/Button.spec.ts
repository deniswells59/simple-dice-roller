...
describe('Button Component', () => {
  test('it calls click handler', () => {
    ...
    const { getByTestId } = render(Button, {
      testId: 'testId',
      onClickHandler: clickHandler.mock,
    });
  ...
  test('it renders primary/secondary styles', () => {
    ...
    const secondaryButton = render(Button, {
      testId: 'secondary',
      primaryAction: false,
    });

    const primaryButtonEl = primaryButton.getByTestId('primary', {
      exact: true,
    });
    const secondaryButtonEl = secondaryButton.getByTestId('secondary', {
      exact: true,
    });
  ...
  test('it extends the button components CSS class', () => {
    const extendedCSSContainer = render(Button, {
      testId: 'extendedClass',
      class: 'bg-yellow',
    });
    const defaultCSSContainer = render(Button, { testId: 'defaultClass' });

    const extendedCSSButton = extendedCSSContainer.getByTestId(
      'extendedClass',
      { exact: true }
    );
    const defaultCSSButton = defaultCSSContainer.getByTestId('defaultClass', {
      exact: true,
    });
