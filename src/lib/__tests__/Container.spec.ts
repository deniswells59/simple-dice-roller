...
describe('Container Component', () => {
  test('it extends the container components CSS class', () => {
    const extendedCSSContainer = render(Container, { testId: 'extendedClass', class: 'bg-yellow' });
    const defaultCSSContainer = render(Container, { testId: 'defaultClass' });

    const extendedCSSContainerEl = extendedCSSContainer.getByTestId('extendedClass', {
      exact: true,
    });
    const defaultCSSContainerEl = defaultCSSContainer.getByTestId('defaultClass', { exact: true });
